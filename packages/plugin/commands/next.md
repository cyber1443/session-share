...
1. `ss_claim` — with the task id if one was given, otherwise let the server pick the best ready task.
...
6. Run the acceptance command from the task. Report the outcome with `ss_report_test`. Passing moves the task toward its PR; failing keeps your lease so you can go again.
